...
/*   Updated: 2024/09/24 13:39:58 by lauger           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	take_colors_value(t_read_file *rf, int p_floor, int p_ceiling)
{
	...
	f_str = deblank(ft_substr(rf->tab_content[p_floor], 1, ft_strlen(rf->tab_content[p_floor]) - 1), 1);
	c_str = deblank(ft_substr(rf->tab_content[p_ceiling], 1, ft_strlen(rf->tab_content[p_ceiling]) - 1), 1);
